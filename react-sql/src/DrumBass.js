...
class DrumBass extends Component {
  state = {
    info: [],
    new_artist: {
      artist: ''
  ...
  getInfo = _ => {
    fetch('http://localhost:4000/music/Drum&Bass')
  ...
  addArtist =_ => {
    ...
    if(new_artist.artist) {
      fetch(`http://localhost:4000/music/Drum&Bass/add?names=${new_artist.artist}`)
        .then(this.getInfo)
        .catch(err => console.error(err))
        new_artist.artist = ''
    }
  ...
  deleteArtist =_ => {
    ...
    fetch(`http://localhost:4000/music/Drum&Bass/delete?names=${delete_artist.artist}`)
      .then(this.getInfo)
      .catch(err => console.error(err))
  }
