...
class Dubstep extends Component {
  ...
  getInfo = _ => {
    fetch('http://localhost:4000/music/Dubstep')
  ...
  addArtist =_ => {
    ...
    if(new_artist.artist) {
      fetch(`http://localhost:4000/music/Dubstep/add?names=${new_artist.artist}`)
        .then(this.getInfo)
        .catch(err => console.error(err))
        new_artist.artist = ''
    }
  ...
  deleteArtist =_ => {
    ...
    fetch(`http://localhost:4000/music/Dubstep/delete?names=${delete_artist.artist}`)
      .then(this.getInfo)
      .catch(err => console.error(err))
  }
